...
test("executes full standard escrow flow from base to arbitrum with raw input 100", async ({
	...
	await page.route("**/quote/request", async (route) => {
		...
		if ((body?.metadata?.exclusiveFor ?? "").toLowerCase() === issuerAddress.toLowerCase()) {
	...
	const currentOrder = await page.evaluate(async () => {
		const { default: store } = await import("/src/lib/state.svelte.ts");
		const { orderToIntent } = await import("/src/lib/core/intent");
		const latest = store.orders.at(-1);
		if (!latest) return null;
		const intent = orderToIntent(latest);
		const inputChain = intent.inputChains()[0];
		return {
			orderId: intent.orderId(),
			inputSettler: latest.inputSettler,
			inputChainId: inputChain.toString()
		};
	});
	expect(currentOrder).not.toBeNull();
	const hasExactInputAmount = await page.evaluate(async (expectedInputRaw) => {
		const { default: store } = await import("/src/lib/state.svelte.ts");
		...
		return sum.toString() === expectedInputRaw;
	}, REQUIRED_INPUT_USDC_RAW);
	expect(hasExactInputAmount).toBe(true);
