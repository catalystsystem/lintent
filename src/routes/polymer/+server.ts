...
export const POST: RequestHandler = async ({ request }) => {
	const { srcChainId, srcBlockNumber, globalLogIndex } = await request.json();
	console.log({srcChainId, srcBlockNumber, globalLogIndex});
	...
	const { result }: { result: number } = requestProof.data;
	...
	if (dat.result.status === "complete") {
		...
		proof = Array.from(proofBytes).map(byte => byte.toString(16).padStart(2, '0')).join('');
	}
