...
export function toStandardBatchCompact(order: StandardOrder, arbiter: `0x${string}`): BatchCompact {
...
export function toMultichainElements(order: MultichainOrder, arbiter: `0x${string}`): Element[] {
	const mandate: CompactMandate = {
		fillDeadline: order.fillDeadline,
		inputOracle: order.inputOracle,
		outputs: order.outputs
