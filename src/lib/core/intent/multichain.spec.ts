...
describe("multichain intent", () => {
	...
	it("constructInputHash builds expected claim structure and validates chain index", () => {
		...
		const expected = keccak256(
			encodePacked(["bytes32[]"], [[additionalChains[0], inputHash, additionalChains[1]]])
		);

		expect(constructInputHash(1n, 1n, [[1n, 10n]], additionalChains)).toBe(expected);
		expect(() => constructInputHash(1n, 3n, [[1n, 10n]], additionalChains)).toThrow(
	...
	it("builds components for every input chain and computes a stable escrow order id", () => {
		...
		const intent = new MultichainOrderIntent(MULTICHAIN_INPUT_SETTLER_ESCROW, order, {
			type: "escrow"
		});
		...
		expect(orderId1.length).toBe(66);
	});

	it("computes compact multichain order ids and uses compact secondaries layout", () => {
		const order = makeMultichainOrder();
		const intent = new MultichainOrderIntent(MULTICHAIN_INPUT_SETTLER_COMPACT, order, {
			type: "compact",
		...
		expect(
			components.every(
				(component) => component.orderComponent.chainIdField === order.inputs[0].chainId
			)
		).toBe(true);
		expect(compactOrderId.startsWith("0x")).toBe(true);
		expect(compactOrderId.length).toBe(66);
	});
});
