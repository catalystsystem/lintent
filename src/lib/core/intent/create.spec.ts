...
import type { CreateIntentOptionsEscrow, TokenContext } from "../types";
import { TEST_NOW_SECONDS, TEST_USER } from "../testing/orderFixtures";
...
function ctx(token: Token, amount: bigint): TokenContext {
	return { token, amount };
}
...
describe("Intent", () => {
	...
	it("detects same-chain when single input and output chains match", () => {
		const intent = new Intent(makeEscrowOptions([ctx(ETH_USDC, 10n)], [ctx(ETH_WETH, 1n)]));
	...
	it("throws when singlechain() is called for multichain input set", () => {
		const intent = new Intent(
			makeEscrowOptions([ctx(ETH_USDC, 10n), ctx(ARB_USDC, 20n)], [ctx(BASE_USDC, 10n)])
		);
	...
	it("order() dispatches to singlechain or multichain intent", () => {
		const single = new Intent(makeEscrowOptions([ctx(ETH_USDC, 1n)], [ctx(ETH_WETH, 1n)])).order();
		const multi = new Intent(
			makeEscrowOptions([ctx(ETH_USDC, 1n), ctx(ARB_USDC, 1n)], [ctx(BASE_USDC, 1n)])
		).order();
