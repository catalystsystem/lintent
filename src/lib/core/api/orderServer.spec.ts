...
describe("parseOrderStatusPayload", () => {
	...
	it("parses a multichain payload nested under data[0].intent", () => {
		...
		if (isStandardOrder(parsed.order)) throw new Error("Expected multichain order");

		expect(parsed.order.inputs.length).toBe(2);
		expect(parsed.order.inputs[1].chainId).toBe(42161n);
		expect(parsed.sponsorSignature).toEqual({ type: "ECDSA", payload: "0xbeef" });
	...
	it("throws when user is not a hex address", () => {
		...
		expect(() => parseOrderStatusPayload(payload)).toThrow("Order payload invalid: order.user");
	});
	...
	it("throws when standard order input tuple is malformed", () => {
		...
		expect(() => parseOrderStatusPayload(payload)).toThrow("Order payload invalid: inputs[0]");
	});
});
